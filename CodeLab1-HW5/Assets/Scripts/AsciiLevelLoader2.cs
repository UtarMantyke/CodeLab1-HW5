...
public class AsciiLevelLoader2 : MonoBehaviour {

	// Use this for initialization
	void Start ()
	{
		string filePath = Application.dataPath + "/level2.txt"; //filePath
		...
		for (int y = 0; y < inputLines.Length; y++) //Loop to go through all the lines
		{
			...
			for (int x = 0; x < line.Length; x++)
			{
				...
				{
					//make a goal
					GameObject player = Instantiate(Resources.Load<GameObject>("Prefabs/Goal"));
					player.transform.position = new Vector2(x - line.Length/2f, inputLines.Length/2f - y);
				}
				else if (line[x] == 'T')
				{
					//make a player
					GameObject player = Instantiate(Resources.Load<GameObject>("Prefabs/Trap"));
					player.transform.position = new Vector2(x - line.Length/2f, inputLines.Length/2f - y);
				}
